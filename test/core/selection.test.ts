...
describe('human-selected workflow candidates', () => {
  let agent: Agent
  ...
  it('checks prepared context after discovery and before the first visit', async () => {
    ...
    ev = evidence(text)
    await activate()
    const run = (await service.startSelectedGraphRun(request(), cap)) as {value: {finished: Promise<{outcome: string}>}}
  ...
  it('records A to B to A as new generations while old work retains its bundle', async () => {
    ...
    const versioned = await compileProcessorGraph({...definition, id: 'b', nodes: [{adapter: 'copy2', id: 'first'}]}, [
  ...
  it('keeps later observations separate and never converts them to dispatch rights', async () => {
    ...
    await service.recordObservation(
      receipt.id,
      {issues: [], runId: 'existing', settlement: {quiescent: true}, status: 'matched', terminal: {outcome: 'unknown'}},
      cap,
    )
  ...
  it('coordinates selected Application Service calls and blocks ordinary surface bypass', async () => {
    ...
    try {
      const thread = app.createThread({formatVersion: 2})
  ...
  it('rejects another prepared catalog within the same fixed scope', async () => {
    ...
    await rejects(service.registerCandidate(changed, compiled, e.plan, e.reports, e.policyText, cap), /scope mismatch/)
