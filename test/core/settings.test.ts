...
describe('loadWorkspaceSettings', () => {
  it('prefers .scribemuse/settings.json over workspace settings.json', async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'scribemuse-settings-'))
    await fs.mkdir(path.join(root, '.scribemuse'))
    await fs.writeFile(path.join(root, '.scribemuse', 'settings.json'), JSON.stringify({provider: 'openai'}))
  ...
  it('throws for invalid JSON', async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'scribemuse-settings-'))
    await fs.mkdir(path.join(root, '.scribemuse'))
    await fs.writeFile(path.join(root, '.scribemuse', 'settings.json'), '{')
  ...
  it('throws for invalid provider values', async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'scribemuse-settings-'))
    await fs.mkdir(path.join(root, '.scribemuse'))
    await fs.writeFile(path.join(root, '.scribemuse', 'settings.json'), JSON.stringify({provider: 'local'}))
