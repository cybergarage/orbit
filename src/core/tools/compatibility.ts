...
import {normalizeToolResult, zodToolInput} from './definition.js'
...
export function adaptInvokableTool(tool: InvokableTool, source: ToolSource): ToolDefinition {
  const input =
    tool.inputSchema === undefined
      ? zodToolInput(tool.schema)
      : {jsonSchema: tool.inputSchema, parse: (value: unknown) => tool.schema.parse(value)}
  return {
