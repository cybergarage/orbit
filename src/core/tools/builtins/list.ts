...
import {DEFAULT_RESULT_LIMIT, displayPath, resolveToolPath} from './shared.js'
...
export function createListTool() {
  return defineBuiltinTool({
    ...
    async execute(input, context) {
      ...
      if (!stat.isDirectory()) throw new Error(`Cannot list non-directory path: ${requestedPath}`)
