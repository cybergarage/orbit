...
import {createGitIgnoreFilter, DEFAULT_IGNORES, DEFAULT_RESULT_LIMIT, displayPath, resolveToolPath} from './shared.js'
...
export function createGlobTool() {
  return defineBuiltinTool({
    ...
    async execute(input, context) {
      ...
      if (!stat.isDirectory()) throw new Error(`Glob base path is not a directory: ${requestedPath}`)
