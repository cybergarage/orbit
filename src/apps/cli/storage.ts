...
export default class StorageCommand extends Command {
  static args = {
    action: Args.string({options: ['initialize', 'inspect', 'recover', 'resume'], required: true}),
  ...
  async run(): Promise<void> {
    ...
    if (args.action !== 'recover' && args.session) this.error('Only recovery takes a session ID')
    if (flags['reviewed-artifacts'] && args.action !== 'resume')
      this.error('Reviewed artifacts are only accepted by resume')
    if (args.action === 'inspect') {
      this.log(JSON.stringify(repository.inspectStorage(), null, 2))
      return
    }
