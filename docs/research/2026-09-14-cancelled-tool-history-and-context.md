...
## Proposal Review Addendum — 2026-09-14

Review baseline is `f218635c4397431239f160f41ad90b0a50a06d9e`. No source or test changes followed the proposal. Public main remains `1cb6f4f2abe3e89e27c1bdb32f1049183ef0e960`. The earlier findings and validation above describe the proposal investigation; the results below belong to this review.

The original six-control fixture was rerun on macOS Node 26.5.0. Two temporary variants each repeated all six controls without changing runtime code or the committed fixture:

- Replace `id: 'read-call'` with `id: 'probe-call'` in the first model response, leaving write's ID unchanged. Approve/deny still continue, while cancellation still leaves the next request failing. Such reused IDs cannot prove which assistant group a call-ID-only HMAC identifies.
- Emit the read and write calls together in the first assistant response; all later responses are the fixed text answer. Approval and denial still continue. On cancellation both Agent and Graph record a succeeded read operation, but neither read nor write tool response is appended. The next request fails before preparation. `ToolRuntime.executeAll` (`src/core/tools/registry.ts`) accumulates results until it returns, so a later exception can prevent an earlier result from reaching the transcript. This is contrary evidence to treating all missing results in a cancelled Run as nondispatch.

These 18 controls verify current mechanics, not the proposed eligibility verifier. The same-group case must remain blocked by the recommended initial scope because its missing read was dispatched. No target-project test, real provider or MCP connection was exercised.

| Reviewed boundary        | Source evidence                                                                                                                                                                                    | Non-binding correction                                                                                                                                                                                                                                                                                             |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Intent versus execution  | `execution/authorization.ts: executePrepared` stores intent before another `valid()`/`run.check()` sequence and may return `cancelled-before-start`. Startup has a separate `mcp-startup` variant. | An intent excludes this initial projection without proving execution. Partition typed startup/direct-Graph/Agent evidence; refuse unexplained records rather than discard them.                                                                                                                                    |
| Synchronous/off path     | `session/context-builder.ts: build` already injects a checkpoint; `agent.ts` uses this context directly when budgeted preparation is disabled.                                                     | The builder is not a raw accessor. A projection-dependent view needs verified preparation even with budgets off. Disabled projection refuses model use; raw observation stays available.                                                                                                                           |
| v3 compatibility         | `agent.ts` Graph/Skill checks, `session/context-policy.ts`, `skills/record.ts`, `session/compaction.ts`, and `processor/graph-inspection.ts` require version 2 explicitly.                         | Distinguish a valid v2 file containing an unresolved raw group from a failed budgeted input; it is not automatically structural corruption. Update the explicit supported-version sets and v3-aware observations together. Old checkpoint/Skill revisions keep their checks; no generic future-version acceptance. |
| Migration and positions  | `session/migration.ts` accepts v1 only and re-encodes every entry into v2. `graph-inspection.ts` indexes all data entries by synchronized high-water positions.                                    | Do not claim current migration preserves bytes. Propose a separate v2-to-v3 byte-preserving stage and distinct intent/artifacts; retain all old entry positions and backups.                                                                                                                                       |
| Preflight ordering       | `processor/graph-execution.ts` dispatches declared nodes in order; an Agent visit is not necessarily the first node.                                                                               | Check prior-history eligibility before effectful preparation/dispatch for the enabled policy, after exact replay lookup. Otherwise an earlier direct tool can run before a later Agent refuses its input.                                                                                                          |
| Observation versus input | The existing readers/Graph inspection can expose incomplete or unavailable evidence without invoking a model.                                                                                      | A structurally valid projection record with missing external proof may be observed as unverified, but cannot authorize model preparation or repair. Same-ID observation does not trigger fresh projection verification/execution.                                                                                  |

The fixed Codex/Pi comparison files from the original investigation were reread locally at the recorded revisions. Their normalization examples still do not establish Orbit's nondispatch evidence or reader/migration compatibility. No new external-version claim is made.

The revised ADR records these costs and additional confirmation conditions. It stays proposed / not-started; no existing acceptance or application admission policy changes. Review validation passed `headers:check`, a fresh `build`, and all 651 existing tests on an isolated copy of the unchanged macOS source; formatter/linter drift was checked separately. Lint retains 56 existing warnings, no errors. The 18 diagnostic controls also passed. The previous maintenance timeout remains unexplained; no timeout occurred in this rerun. Linux results above are prior evidence, not a new Linux run. These checks do not implement or qualify the proposed projection, migration or provider acceptance.
